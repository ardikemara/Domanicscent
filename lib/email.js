...
// Email "kamu di-approve jadi affiliate" (dikirim dari admin pas approve).
export function affiliateApprovedEmail({ name, slug }) {
  const url = siteUrl();
  const link = `${url}/r/${slug}`;
  const subject = "Selamat, kamu resmi jadi affiliate Domanic!";
  const html = `<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${esc(subject)}</title></head>
<body style="margin:0;padding:0;background:${C.paper};font-family:Arial,Helvetica,sans-serif;color:${C.charcoal}">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.paper}"><tr><td align="center" style="padding:28px 16px">
  <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#fff;border:1px solid ${C.cream};border-radius:14px">
    ...
    <tr><td style="padding:16px 34px 0">
      <p style="font-size:15px;line-height:1.7;margin:0 0 12px">Halo ${esc(name || "")},</p>
      <p style="font-size:14px;line-height:1.7;margin:0 0 12px">Akun affiliate kamu udah <b>di-approve</b>. Ini link toko kamu:</p>
      <div style="background:${C.cream};border:1px dashed ${C.beige};border-radius:10px;padding:14px;text-align:center;margin:0 0 14px">
        <a href="${esc(link)}" style="font-size:15px;color:${C.umber};font-weight:bold;text-decoration:none">${esc(link)}</a>
      </div>
      <p style="font-size:14px;line-height:1.7;margin:0 0 8px">Cara kerjanya:</p>
      <ul style="font-size:13px;line-height:1.8;margin:0 0 14px;padding-left:18px;color:${C.charcoal}">
        <li>Share link itu (atau tambah <b>?ref=${esc(slug)}</b> di link produk mana pun).</li>
        <li>Tiap penjualan lunas dari link kamu = <b>komisi 15%</b> dari harga produk.</li>
        <li>Komisi bisa dicairkan setelah 7 hari, minimal saldo Rp 250.000, proses 3 hari kerja.</li>
      </ul>
      <p style="font-size:13px;line-height:1.7;color:${C.taupe};margin:0 0 20px">Dashboard affiliate buat pantau penjualanmu lagi disiapkan, nyusul ya. Sementara itu link kamu udah aktif dan komisi udah mulai kehitung.</p>
    </td></tr>
  </table>
  <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%"><tr><td style="padding:18px 30px;text-align:center">
    <p style="font-size:11px;line-height:1.6;color:${C.taupe};margin:0">DOMANIC, extrait de parfum, dibikin pelan di Indonesia.</p>
...
// Email magic link login affiliate (berlaku 15 menit).
export function affiliateLoginEmail({ name, loginUrl }) {
  const subject = "Link masuk dashboard affiliate Domanic";
  const html = `<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${esc(subject)}</title></head>
<body style="margin:0;padding:0;background:${C.paper};font-family:Arial,Helvetica,sans-serif;color:${C.charcoal}">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.paper}"><tr><td align="center" style="padding:28px 16px">
  <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#fff;border:1px solid ${C.cream};border-radius:14px">
    ...
    <tr><td style="padding:16px 34px 26px">
      <p style="font-size:15px;line-height:1.7;margin:0 0 12px">Halo ${esc(name || "")},</p>
      <p style="font-size:14px;line-height:1.7;margin:0 0 18px">Klik tombol di bawah buat masuk ke dashboard affiliate kamu. Link ini berlaku <b>15 menit</b>.</p>
      <div style="text-align:center;margin:0 0 18px">
        <a href="${esc(loginUrl)}" style="display:inline-block;background:${C.umber};color:${C.paper};text-decoration:none;font-size:14px;letter-spacing:2px;padding:13px 30px;border-radius:999px">MASUK DASHBOARD</a>
      </div>
      <p style="font-size:12px;line-height:1.6;color:${C.taupe};margin:0">Kalau kamu nggak ngerasa minta link ini, abaikan aja email ini. Nggak ada yang berubah di akunmu.</p>
    </td></tr>
  </table>
</td></tr></table>
</body></html>`;
...
// Kerangka email affiliate sederhana (judul + isi paragraf).
function affiliateShellEmail(subject, bodyHtml) {
  const html = `<!doctype html><html lang="id"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${esc(subject)}</title></head>
<body style="margin:0;padding:0;background:${C.paper};font-family:Arial,Helvetica,sans-serif;color:${C.charcoal}">
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${C.paper}"><tr><td align="center" style="padding:28px 16px">
  <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="max-width:560px;width:100%;background:#fff;border:1px solid ${C.cream};border-radius:14px">
    ...
    <tr><td style="padding:16px 34px 26px">${bodyHtml}</td></tr>
  </table>
</td></tr></table>
</body></html>`;
  return { subject, html };
...
// Email konfirmasi pengajuan pencairan (ke affiliate).
export function payoutRequestedEmail({ name, amount }) {
  return affiliateShellEmail(
    "Pengajuan pencairan komisimu kami terima",
    `<p style="font-size:15px;line-height:1.7;margin:0 0 12px">Halo ${esc(name || "")},</p>
     <p style="font-size:14px;line-height:1.7;margin:0 0 12px">Pengajuan pencairan komisi sebesar <b>${rp(amount)}</b> udah kami terima. Kami proses maksimal <b>3 hari kerja</b> ke rekening yang terdaftar di akunmu.</p>
     <p style="font-size:13px;line-height:1.7;color:${C.taupe};margin:0">Kamu bakal dapat email lagi begitu transfernya dikirim. Status pengajuan juga bisa dipantau di dashboard affiliate.</p>`
  );
}

// Email pencairan sudah ditransfer (ke affiliate).
export function payoutPaidEmail({ name, amount, bankName, bankAccountMasked }) {
  return affiliateShellEmail(
    "Pencairan komisimu sudah ditransfer 🎉",
    `<p style="font-size:15px;line-height:1.7;margin:0 0 12px">Halo ${esc(name || "")},</p>
     <p style="font-size:14px;line-height:1.7;margin:0 0 12px">Komisi sebesar <b>${rp(amount)}</b> udah kami transfer ke rekening <b>${esc(bankName || "")} ${esc(bankAccountMasked || "")}</b>.</p>
     <p style="font-size:13px;line-height:1.7;color:${C.taupe};margin:0">Makasih udah bantu nyebarin Domanic. Gas lanjut share link-mu!</p>`
  );
}
